...
export function OrderDetailsPage() {
  ...
  if (!order) {
    ...
    return (
      <IonContent className="ion-padding">
        <div className="text-center py-16 space-y-6 pt-12">
    ...
    <IonContent className="ion-padding">
      ...
      {order.status !== 'cancelled' && (
        <IonCard className="border border-neutral-200">
          <IonCardContent>
            <h2 className="text-base font-semibold mb-6 text-neutral-900">Order Progress</h2>
          ...
          <div className="relative">
            ...
            {/* Progress Line */}
            <div className="absolute top-6 left-6 right-6 h-1 bg-neutral-200 rounded-full -z-10">
              <div 
          ...
          <div className="mt-6 p-4 bg-yellow-50 rounded-xl border border-yellow-200">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-yellow-500 rounded-lg flex items-center justify-center">
                <span className="text-neutral-900 text-sm">⏱️</span>
              </div>
      ...
      <IonCard className="border border-neutral-200">
        <IonCardContent>
          <h2 className="text-base font-semibold mb-4 text-neutral-900">Order Items</h2>
                  ...
                  {item.extras && item.extras.length > 0 && (
                    <div className="ml-4 mt-1">
                      {item.extras.map((extra: any, extraIndex: number) => (
                         <div key={extraIndex} className="text-sm text-neutral-600">
                          + {extra.name}
                        </div>
                      ))}
                    </div>
      ...
      <IonCard className="border border-neutral-200">
        <IonCardContent>
          <h2 className="text-base font-semibold mb-4 text-neutral-900">Order Details</h2>
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-neutral-900">{formatDateTime(order.pickup_time)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-neutral-600">Location Sharing</span>
              <span className="text-neutral-900">{order.share_location ? 'Enabled' : 'Disabled'}</span>
            </div>
          </div>
        </IonCardContent>
      </IonCard>
