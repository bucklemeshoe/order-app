    ...
    <IonContent className="ion-padding">
      <div className="space-y-4 pt-12">
        ...
        {notifications.length === 0 ? (
          <div className="text-center text-neutral-600 py-16">No notifications yet</div>
        ) : (
          <div className="space-y-3">
            {notifications.map((n) => (
              <IonCard key={n.id} className={n.seen ? 'border border-neutral-200' : 'border border-yellow-300'}>
                <IonCardContent>
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-semibold text-neutral-900">{n.title}</h3>
                      <p className="text-neutral-600 text-sm">{n.message}</p>
                    </div>
                    <span className="text-xs text-neutral-500">{new Date(n.createdAt).toLocaleString('en-ZA')}</span>
                  </div>
                </IonCardContent>
              </IonCard>
            ))}
